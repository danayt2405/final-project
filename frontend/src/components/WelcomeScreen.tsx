...
export function WelcomeScreen({ language, role }: WelcomeScreenProps) {
  ...
  const subtitleText = language === 'am'
    ? 'የመረጃ አውታረ መረብ ደህንነት አስተዳደር የቅሬታ አስተዳደር ስርዓት'
    : 'Information Network Security Administration Complaint Management System';

  const roleGreeting = role === 'ADMIN'
    ? (language === 'am' ? 'የአስተዳዳሪ ዳሽቦርድ' : 'Administrator Dashboard')
    : (language === 'am' ? 'የተጠቃሚ ዳሽቦርድ' : 'User Dashboard');
      ...
      >
        ...
        >
          <div className="w-32 h-32 bg-white rounded-full flex items-center justify-center shadow-2xl p-6">
            <img src={insaLogo} alt="INSA Logo" className="w-full h-full object-contain" />
          </div>
        </motion.div>
